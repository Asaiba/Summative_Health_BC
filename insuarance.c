...
/* ----------------------------- Small SHA-256 implementation -----------------------------
    Public-domain, small footprint implementation adapted for embedding.
    Produces 32-byte hash. Output utility below converts to hex string.
---------------------------------------------------------------------------------------*/
...
void sha256(const u8 *message, size_t len, u8 hash[32]) {
    ...
    u8 *msg = calloc(new_len + 8, 1);
    memcpy(msg, message, len);
    msg[len] = 0x80;
    u64 bits_len = (u64)len * 8;
    ...
    for (size_t offset = 0; offset < new_len + 8; offset += 64) {
        ...
        for (int i = 0; i < 64; i++) {
            u32 t1 = hh + EP1(e) + CH(e,f,g) + k[i] + w[i];
 ...
 * e.g., difficulty=3 requires hash starting with "000" (12 leading zero bits approx).
 */
int difficulty = 3;
